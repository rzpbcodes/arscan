...
class IntegerOverflow(AbstractDetector):
    ...
    def op_vars_in_call(_ir, operation, indexs):
        checked_vars = IntegerOverflow.checked_vars(_ir.function)
        ...
        for node in _ir.function.nodes:
            for ir in node.irs:
                if isinstance(ir, Binary) and ir.type in operation:
                    ...
                    issue_params = IntegerOverflow.unsafe_params(operands, node.variables_written, func_params, checked_vars)
    ...
    def _detect(self) -> List[Output]:
        """"""
        results: List[Output] = []
        ops = [BinaryType.ADDITION, BinaryType.MULTIPLICATION, BinaryType.POWER]
        ...
        for contract in self.compilation_unit.contracts:
            ...
            for function in contract.functions:
                if function.contract_declarer != contract:
                    continue
                if not function.visibility in IntegerOverflow.accessible:
                    continue

                flagged_nodes = set()
                func_params = set(function.parameters)
                checked_vars = IntegerOverflow.checked_vars(function)

                for node in function.nodes:
                    for ir in node.irs:
                        if isinstance(ir, (LibraryCall, InternalCall)):
                            if isinstance(ir.function, Function):
                                unsafe_vars = IntegerOverflow.op_vars_in_call(ir, ops, [0,1])
                                issue_params = IntegerOverflow.params_dependency(unsafe_vars, ir.function.parameters, ir.arguments, function.parameters, checked_vars)
                                if len(issue_params) > 0:
                                    flagged_nodes.add(node)
                                    break
                        elif isinstance(ir, Binary) and ir.type in ops:
                            issue_params = IntegerOverflow.unsafe_params(ir.get_variable, node.variables_written, func_params, checked_vars)
                            if len(issue_params) > 0:
                                flagged_nodes.add(node)
                                break

                if flagged_nodes:
                    info: DETECTOR_INFO = [function, " may overflow in add, multiply and power operations:- \n"]
